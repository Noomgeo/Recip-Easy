...
class Recipes:
    ...
    def add_recipe(self, data, user_id):
        '''Add a recipe to the database. Use the given dictionary of data as well as the given user ID as data for the new row.'''
        new_tuple = (user_id, data['title'], data['image_URL'], data['description'], data['ingredients'])
        query = "INSERT INTO recipe (user_id, title, image_URL, description, ingredients) VALUES(?, ?, ?, ?, ?)"
